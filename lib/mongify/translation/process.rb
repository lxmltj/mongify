  ...
  class Translation
    ...
    module Process
      ...
      def copy_data
        ...
        if copy_tables_non_parallel
          copy_tables_non_parallel.each do |t|
            ...
            Parallel.each((1..pages), in_processes: self.processes, progress:"Copying #{t.name} (CPUs: #{self.processes})") do |page|
      ...
      def update_reference_ids
        Parallel.each(self.copy_tables, in_threads: self.processes, progress:"Updating References Parallel (CPUs: #{self.processes}, Tables: #{self.copy_tables.count})") do |t|
